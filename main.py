...
_font18    = pygame.font.SysFont(None, 18)
_font19    = pygame.font.SysFont(None, 19)
_font20    = pygame.font.SysFont(None, 20)
_font22    = pygame.font.SysFont(None, 22)
_font26    = pygame.font.SysFont(None, 26)
...
def _is_web():
    ...
    try:
        ...
        if hasattr(platform, "window"):
            return True
        # pygbag also sets sys.platform to 'emscripten'
        import sys as _sys
        if _sys.platform in ("emscripten", "wasm"):
            return True
        return False
    except Exception:
        return False

# Cache result — _is_web() is called many times per frame
_WEB_MODE = _is_web()
...
def _web_save(slot, data_str):
    try:
        ...
        # Try emscripten storage fallback
        try:
            import js
            js.localStorage.setItem(_ls_key(slot), data_str)
            return True
        except Exception:
            return False
...
def _web_load(slot):
    try:
        ...
        try:
            import js
            val = js.localStorage.getItem(_ls_key(slot))
...
def _web_delete(slot):
    try:
        ...
        try:
            import js
            js.localStorage.removeItem(_ls_key(slot))
        except Exception:
            pass
...
def _deserialise_game(raw):
    ...
    # Rebuild transient effect objects from saved bouncer state
    for b in bouncers:
        if b.implosion_enabled:
            implosion_effects.append(ImplosionEffect(b))
        if b.gravity_enabled and b.gravity_purchases > 0:
            gravity_effects.append(GravityWellEffect(b))
    # Rebuild illuminate effects
    n_ill = sum(1 for b in bouncers if b.illuminate_enabled)
    for ei in range(n_ill):
        eff = IlluminateEffect()
        eff.cx = int(GAME_WIDTH * (ei + 1) / (n_ill + 1))
        eff.cy = HEIGHT // 2
        illuminate_effects.append(eff)
    # Rebuild factories
    n_fac = max(0, max((sum(1 for it in b.shop_data if it["action"]=="factory" and it["bought"]>0)
                        for b in bouncers), default=0))
    fac_bought = sum(it["bought"] for b in bouncers for it in b.shop_data if it["action"]=="factory")
    if fac_bought > 0:
        for fi in range(fac_bought):
            fx2 = int(GAME_WIDTH * (fi + 1) / (fac_bought + 1))
            factories.append(Factory(fx2))
    # Rebuild 3D mode
    if any(b.mode3d_enabled for b in bouncers):
        mode3d_active = True
...
# speed/size capped at 20. Income upgrades (bubble→faucet) UNCAPPED.
NW_SHOP_MAX_STAT  = 20   # speed / size cap
NW_SHOP_MAX_MONEY = 999  # income upgrades — effectively uncapped
NW_UNLOCK_THRESH  = 3    # buys of tier N needed to unlock tier N+1

# Ordered cheapest→most expensive (bubble→ducky→hottub→disco→faucet)
NW_SHOP_TRACKS = [
    {"action": "nw_speed",  "name": "SPEED",
     "accent": (120, 200, 255), "bg": (30, 35, 52),
     "desc": "+5% bouncer speed  (cap 20)",
     "cap": NW_SHOP_MAX_STAT},
    {"action": "nw_size",   "name": "SIZE",
     "accent": (100, 220, 120), "bg": (28, 44, 30),
     "desc": "+5% bouncer size  (cap 20)",
     "cap": NW_SHOP_MAX_STAT},
    {"action": "nw_bubble", "name": "BUBBLE BATH",
     "accent": (140, 210, 255), "bg": (18, 36, 54),
     "desc": "+8G/s  (no cap — stack it)"},
    {"action": "nw_ducky",  "name": "RUBBER DUCKY",
     "accent": (255, 215, 30),  "bg": (46, 40, 8),
     "desc": "+15G/bounce  (no cap)"},
    {"action": "nw_hottub", "name": "HOT TUB",
     "accent": (255, 110, 70),  "bg": (52, 22, 14),
     "desc": "+40G/s  (no cap)"},
    {"action": "nw_disco",  "name": "DISCO FEVER",
     "accent": (255, 60, 220),  "bg": (46, 10, 46),
     "desc": "+25G/bounce + floor party  (no cap)"},
    {"action": "nw_faucet", "name": "GOLDEN FAUCET",
     "accent": (255, 200, 30),  "bg": (48, 36, 4),
     "desc": "+100G/s  (no cap — pure gold)"},
]

def _nw_cap(action):
    for t in NW_SHOP_TRACKS:
        if t["action"] == action:
            return t.get("cap", NW_SHOP_MAX_MONEY)
    return NW_SHOP_MAX_MONEY

# bought[action] = total purchases on that track (0-20)
_nw_shop_bought  = {"nw_speed": 0, "nw_size": 0, "nw_bubble": 0,
                    "nw_ducky": 0, "nw_hottub": 0, "nw_disco": 0, "nw_faucet": 0}
# disabled[action] = True means visual effect toggled off
_nw_effect_off   = {"nw_speed": False, "nw_size": False, "nw_bubble": False,
                    "nw_ducky": False,  "nw_hottub": False, "nw_disco": False,
                    "nw_faucet": False}

# ── NW passive income timers ──────────────────────────────────────────────
_nw_income_timer  = 0   # ms, tracks last passive income tick

# ── Bubble state: [x, y, r, vx, vy, hue, alpha, wobble_phase] ────────────
_nw_bubbles: list = []
_nw_bubble_timer  = 0.0   # seconds since last bubble spawn

# ── Ducky state: angle around bouncer ────────────────────────────────────
_nw_ducky_angle   = 0.0
_nw_ducky_wobble  = 0.0

# ── Hot tub steam: [x, y, vy, size, alpha, decay] ────────────────────────
_nw_steam: list   = []
_nw_steam_timer   = 0.0

# ── Disco tile state: active_tile index, flash_timer ─────────────────────
_nw_disco_flash   = []   # list of (col_idx, flash_life)
_nw_disco_timer   = 0.0
NW_DISCO_COLS = [(255,40,120),(40,255,200),(255,200,40),(120,40,255),(40,200,255),(255,120,40)]

# ── Faucet drip: [x, y, vy, size, alpha] ─────────────────────────────────
_nw_faucet_drips: list = []
_nw_faucet_timer  = 0.0
...
def _nw_passive_income_per_sec():
    """Return total passive green-coin income per second from all NW upgrades."""
    total = 0
    if not _nw_effect_off["nw_bubble"]:
        total += 8 * _nw_shop_bought["nw_bubble"]    # 8G/s per level
    if not _nw_effect_off["nw_hottub"]:
        total += 40 * _nw_shop_bought["nw_hottub"]   # 40G/s per level
    if not _nw_effect_off["nw_faucet"]:
        total += 100 * _nw_shop_bought["nw_faucet"]  # 100G/s per level
    return total

def _nw_bounce_bonus():
    """Extra green coins earned on each NW bouncer wall bounce."""
    bonus = 1   # base
    duck_lvl = _nw_shop_bought["nw_ducky"]
    if duck_lvl > 0 and not _nw_effect_off["nw_ducky"]:
        bonus += 15 * duck_lvl                       # 15G/bounce per level
    disco_lvl = _nw_shop_bought["nw_disco"]
    if disco_lvl > 0 and not _nw_effect_off["nw_disco"]:
        bonus += 25 * disco_lvl                      # 25G/bounce per level
    return bonus

def _nw_update_effects(dt, t, bo_cx, bo_cy, floor_y, W, H, hit_wall):
    """Update all NW visual effect state (bubbles, ducky angle, steam, disco, faucet drips)."""
    global _nw_bubble_timer, _nw_steam_timer, _nw_disco_timer, _nw_faucet_timer
    global _nw_ducky_angle, _nw_ducky_wobble, green_coins, _nw_income_timer

    now_ms = pygame.time.get_ticks()

    # ── Passive income tick ───────────────────────────────────────────────
    if now_ms - _nw_income_timer >= 1000:
        ticks = (now_ms - _nw_income_timer) // 1000
        green_coins += _nw_passive_income_per_sec() * ticks
        _nw_income_timer += ticks * 1000

    # ── Bubbles ──────────────────────────────────────────────────────────
    lvl_b = _nw_shop_bought["nw_bubble"]
    if lvl_b > 0 and not _nw_effect_off["nw_bubble"]:
        _nw_bubble_timer += dt
        spawn_rate = max(0.15, 0.8 - lvl_b * 0.04)
        while _nw_bubble_timer >= spawn_rate:
            _nw_bubble_timer -= spawn_rate
            if len(_nw_bubbles) < 40:
                bx = random.uniform(20, W - 20)
                by = random.uniform(floor_y - 80, floor_y - 10)
                br = random.uniform(8, 22)
                _nw_bubbles.append([bx, by, br,
                                     random.uniform(-18, 18),
                                     random.uniform(-35, -15),
                                     random.randint(0, 360),
                                     random.uniform(0.6, 1.0),
                                     random.uniform(0, math.pi * 2)])
    i = 0
    while i < len(_nw_bubbles):
        b = _nw_bubbles[i]
        b[0] += b[3] * dt
        b[1] += b[4] * dt
        b[4] *= 0.994              # slow vertical drift
        b[3] += random.uniform(-8, 8) * dt   # wobble horizontal
        b[7] += 2.0 * dt           # wobble phase
        b[6] -= dt * 0.06          # fade out
        if b[6] <= 0 or b[1] < -b[2] * 2:
            _nw_bubbles[i] = _nw_bubbles[-1]; _nw_bubbles.pop()
        else:
            i += 1

    # ── Ducky orbit ───────────────────────────────────────────────────────
    lvl_d = _nw_shop_bought["nw_ducky"]
    if lvl_d > 0 and not _nw_effect_off["nw_ducky"]:
        _nw_ducky_angle += dt * (1.2 + lvl_d * 0.15)
        _nw_ducky_wobble = math.sin(t * 4.0) * 4.0

    # ── Steam from hot tub ────────────────────────────────────────────────
    lvl_h = _nw_shop_bought["nw_hottub"]
    if lvl_h > 0 and not _nw_effect_off["nw_hottub"]:
        _nw_steam_timer += dt
        srate = max(0.06, 0.25 - lvl_h * 0.015)
        tub_x = int(W * 0.82)
        tub_y = floor_y - 28
        while _nw_steam_timer >= srate:
            _nw_steam_timer -= srate
            if len(_nw_steam) < 60:
                _nw_steam.append([
                    float(tub_x + random.randint(-30, 30)),
                    float(tub_y),
                    random.uniform(-28, -14),
                    random.uniform(8, 18),
                    random.uniform(0.7, 1.0),
                    random.uniform(0.25, 0.55),
                ])
    i = 0
    while i < len(_nw_steam):
        s = _nw_steam[i]
        s[0] += random.uniform(-10, 10) * dt
        s[1] += s[2] * dt
        s[2] *= 0.98
        s[4] -= s[5] * dt
        s[3] += 4 * dt
        if s[4] <= 0 or s[1] < 0:
            _nw_steam[i] = _nw_steam[-1]; _nw_steam.pop()
        else:
            i += 1

    # ── Disco floor flashes on bounce ────────────────────────────────────
    lvl_di = _nw_shop_bought["nw_disco"]
    if hit_wall and lvl_di > 0 and not _nw_effect_off["nw_disco"]:
        for _ in range(min(4, lvl_di + 1)):
            cidx = random.randint(0, len(NW_DISCO_COLS) - 1)
            _nw_disco_flash.append([cidx, 1.0])
    i = 0
    while i < len(_nw_disco_flash):
        _nw_disco_flash[i][1] -= dt * 1.8
        if _nw_disco_flash[i][1] <= 0:
            _nw_disco_flash[i] = _nw_disco_flash[-1]; _nw_disco_flash.pop()
        else:
            i += 1

    # ── Faucet gold drips ─────────────────────────────────────────────────
    lvl_f = _nw_shop_bought["nw_faucet"]
    if lvl_f > 0 and not _nw_effect_off["nw_faucet"]:
        _nw_faucet_timer += dt
        frate = max(0.05, 0.35 - lvl_f * 0.025)
        faucet_x = int(W * 0.12)
        faucet_y = 90
        while _nw_faucet_timer >= frate:
            _nw_faucet_timer -= frate
            if len(_nw_faucet_drips) < 50:
                _nw_faucet_drips.append([
                    float(faucet_x + random.randint(-4, 4)),
                    float(faucet_y),
    ...
    i = 0
    while i < len(_nw_faucet_drips):
        d = _nw_faucet_drips[i]
        d[1] += d[2] * dt
        d[4] -= dt * 1.2
        if d[4] <= 0 or d[1] > floor_y:
            _nw_faucet_drips[i] = _nw_faucet_drips[-1]; _nw_faucet_drips.pop()
        else:
            i += 1

def _nw_tier_price(action, tier):
    """Price to buy tier (0-indexed). ROI ≈ 50-80 seconds per purchase at any tier."""
    bases = {
        "nw_speed":  4,     # utility — cheap
        "nw_size":   6,     # utility — cheap
        "nw_bubble": 25,    # 8G/s → ~3s payback at base, scales with cost
        "nw_ducky":  50,    # 15G/bounce → fast payback
        "nw_hottub": 200,   # 40G/s → 5s payback at base
        "nw_disco":  350,   # 25G/bounce → mid payback
        "nw_faucet": 500,   # 100G/s → 5s payback at base
    }
    growths = {
        "nw_speed":  1.28, "nw_size":   1.28,
        "nw_bubble": 1.32, "nw_ducky":  1.35,
        "nw_hottub": 1.38, "nw_disco":  1.40,
        "nw_faucet": 1.42,
    }
    base   = bases.get(action, 10)
    growth = growths.get(action, 1.35)
    return max(1, math.ceil(base * (growth ** tier)))
...
def _nw_tier_unlocked(action):
    ...
    cap = _nw_cap(action)
    return min(cap, 1 + _nw_shop_bought[action] // NW_UNLOCK_THRESH)

def _nw_can_buy(action):
    return _nw_shop_bought[action] < _nw_cap(action)
...
def _nw_draw_bathroom(surf, W, H, t, nw_bounce_particles=None, mx=0, my=0):
    ...
    # ── DISCO TILES flash effect on floor ───────────────────────────────
    if _nw_shop_bought["nw_disco"] > 0 and not _nw_effect_off["nw_disco"]:
        di_lvl = _nw_shop_bought["nw_disco"]
        FTW2, FTH2 = 40, 20
        tile_cols = [(r, c) for r in range(int((H - floor_y) / FTH2) + 3)
                     for c in range(int(W / FTW2) + 3)]

        # Richer tile flashes — glow bleed between tiles
        for fi, flash in enumerate(_nw_disco_flash):
            cidx, life = flash
            col_d = NW_DISCO_COLS[cidx % len(NW_DISCO_COLS)]
            alpha_d = int(life * 220)
            seed_d = (fi * 13 + cidx * 7) % max(1, len(tile_cols))
            r_d, c_d = tile_cols[seed_d]
            tx_d = c_d * FTW2 + (r_d % 2) * (FTW2 // 2)
            ty_d = floor_y + r_d * FTH2
            # Tile fill
            fs = pygame.Surface((FTW2 - 2, FTH2 - 2), pygame.SRCALPHA)
            fs.fill((*col_d, alpha_d))
            surf.blit(fs, (tx_d + 1, ty_d + 1))
            # Glow bleed to adjacent tiles
            for nx_off, ny_off in [(-FTW2, 0), (FTW2, 0), (0, -FTH2), (0, FTH2)]:
                gs7 = pygame.Surface((FTW2 - 2, FTH2 - 2), pygame.SRCALPHA)
                gs7.fill((*col_d, alpha_d // 4))
                surf.blit(gs7, (tx_d + 1 + nx_off, ty_d + 1 + ny_off))

        # ── Disco mirror ball ─────────────────────────────────────────────
        db_x = NW_GAME_W // 2 - 100
        db_y  = 28
        ball_r = 16 + min(di_lvl, 5) * 2  # grows with level
        # Hanging chain
        for chain_y in range(0, db_y - ball_r, 5):
            link_c = (160, 160, 170) if (chain_y // 5) % 2 == 0 else (130, 130, 140)
            pygame.draw.line(surf, link_c, (db_x - 1, chain_y), (db_x + 1, chain_y + 4), 2)

        # Ball shadow on ceiling
        shd_s = pygame.Surface((ball_r * 3, 8), pygame.SRCALPHA)
        pygame.draw.ellipse(shd_s, (0, 0, 0, 40), (0, 2, ball_r * 3, 6))
        surf.blit(shd_s, (db_x - ball_r * 3 // 2, db_y + ball_r - 2))

        # Ball body — layered facets
        for ring in range(6):
            n_segs = 8 + ring * 2
            ring_y_off = -ball_r + ring * (ball_r * 2 // 5)
            ring_r = int(math.sqrt(max(0, ball_r**2 - ring_y_off**2)))
            for seg in range(n_segs):
                ang1 = seg * math.pi * 2 / n_segs + t * 1.6 + ring * 0.3
                ang2 = ang1 + math.pi * 2 / n_segs
                # Facet colour
                bri = int(80 + 140 * abs(math.sin(ang1 + t * 2.0 + ring * 0.5)))
                col_f = NW_DISCO_COLS[abs(int(ang1 * 2 + t + ring)) % len(NW_DISCO_COLS)]
                mx_f = int(db_x + (math.cos(ang1) + math.cos(ang2)) / 2 * ring_r)
                my_f = int(db_y + ring_y_off + ring * 1)
                facet_size = max(2, ring_r // 5)
                facet_col = (min(255, int(col_f[0]*bri/255)),
                             min(255, int(col_f[1]*bri/255)),
                             min(255, int(col_f[2]*bri/255)))
                pygame.draw.rect(surf, facet_col,
                                 (mx_f - facet_size // 2, my_f - facet_size // 2,
                                  facet_size, facet_size))
        # Ball outline
        pygame.draw.circle(surf, (200, 200, 210), (db_x, db_y), ball_r, 2)
        # Centre shine
        pygame.draw.circle(surf, (255, 255, 255), (db_x - ball_r // 4, db_y - ball_r // 4), max(2, ball_r // 6))

        # ── Spotlight beams sweeping the room ─────────────────────────────
        n_beams = min(6, 2 + di_lvl)
        for bi in range(n_beams):
            ba = bi * math.pi * 2 / n_beams + t * 0.7
            col_beam = NW_DISCO_COLS[(bi + int(t * 2)) % len(NW_DISCO_COLS)]
            beam_a = int(55 + 35 * math.sin(t * 2.8 + bi * 1.1))
            beam_end_x = int(db_x + math.cos(ba) * 200)
            beam_end_y = floor_y
            # Draw beam as two lines (no full-screen surface)
            pygame.draw.line(surf, (min(255,col_beam[0]//2), min(255,col_beam[1]//2), min(255,col_beam[2]//2)),
                             (db_x, db_y), (beam_end_x, beam_end_y), 1)
            # Floor spotlight ellipse — small clipped surface
            spot_r = max(8, 12 + int(8 * math.sin(t * 1.5 + bi)))
            sw2 = max(4, spot_r * 4); sh2 = max(2, spot_r * 2)
            spot_s = pygame.Surface((sw2, sh2), pygame.SRCALPHA)
            pygame.draw.ellipse(spot_s, (*col_beam, min(120, beam_a + 40)), (0, 0, sw2, sh2))
            surf.blit(spot_s, (beam_end_x - sw2 // 2, beam_end_y - sh2 // 2))

        # ── Income label ──────────────────────────────────────────────────
        di_inc = 25 * di_lvl
        di_s = _font19.render(f"+{di_inc}G/bounce", True, (255, 80, 220))
        surf.blit(di_s, (db_x - 30, db_y + ball_r + 8))

    # ── HOT TUB (right side of bathroom) ────────────────────────────────
    if _nw_shop_bought["nw_hottub"] > 0 and not _nw_effect_off["nw_hottub"]:
        tub_x = int(W * 0.82)
        tub_w = 120; tub_h = 58
        tub_left = tub_x - tub_w // 2
        tub_top  = floor_y - tub_h
        ht_lvl   = _nw_shop_bought["nw_hottub"]

        # ── Tub surround (wooden decking strips) ──────────────────────────
        deck_col = (148, 98, 58)
        deck_hi  = (172, 118, 72)
        for dk in range(3):
            dy2 = tub_top - 14 + dk * 5
            pygame.draw.rect(surf, deck_col, (tub_left - 14, dy2, tub_w + 28, 5), border_radius=1)
            pygame.draw.line(surf, deck_hi, (tub_left - 14, dy2), (tub_left + tub_w + 14, dy2), 1)
        # Side panels
        for sx2, sw2 in [(tub_left - 14, 14), (tub_left + tub_w, 14)]:
            pygame.draw.rect(surf, deck_col, (sx2, tub_top, sw2, tub_h + 14), border_radius=2)
            for stripe_y in range(tub_top, floor_y + 12, 5):
                pygame.draw.line(surf, deck_hi, (sx2, stripe_y), (sx2 + sw2, stripe_y), 1)

        # ── Tub shell ─────────────────────────────────────────────────────
        pygame.draw.rect(surf, (210, 192, 178), (tub_left - 4, tub_top - 4, tub_w + 8, tub_h + 12), border_radius=14)
        pygame.draw.rect(surf, (236, 220, 208), (tub_left, tub_top, tub_w, tub_h), border_radius=12)
        pygame.draw.rect(surf, (168, 148, 132), (tub_left, tub_top, tub_w, tub_h), 2, border_radius=12)

        # ── Animated glowing water ─────────────────────────────────────────
        wt = t * 1.4
        wc1 = (int(40 + 30*math.sin(wt)),     int(130 + 40*math.sin(wt*0.9)),    min(255, 180 + 30*int(ht_lvl>2)))
        wc2 = (int(60 + 30*math.sin(wt+1.2)), int(150 + 40*math.sin(wt*0.9+1)), min(255, 200 + 20*int(ht_lvl>3)))
        pygame.draw.ellipse(surf, wc2, (tub_left + 4, tub_top + 6, tub_w - 8, tub_h - 22))
        pygame.draw.ellipse(surf, wc1, (tub_left + 8, tub_top + 10, tub_w - 16, tub_h - 28))

        # ── LED strip glow around rim (brighter at higher levels) ─────────
        if ht_lvl >= 3:
            led_a = int(80 + 40 * math.sin(t * 2.2))
            led_col_idx = int(t * 60) % len(NW_DISCO_COLS)
            led_c = NW_DISCO_COLS[led_col_idx]
            led_s = pygame.Surface((tub_w + 8, tub_h + 12), pygame.SRCALPHA)
            pygame.draw.rect(led_s, (*led_c, led_a), (0, 0, tub_w + 8, tub_h + 12), 4, border_radius=14)
            surf.blit(led_s, (tub_left - 4, tub_top - 4))

        # ── Bubble jets ───────────────────────────────────────────────────
        for bub in range(min(6, 2 + ht_lvl)):
            bub_phase = (t * (0.7 + bub * 0.15)) % 1.0
            bub_r = int(3 + 7 * bub_phase)
            bub_a = int(200 * (1.0 - bub_phase))
            bub_x2 = tub_left + 12 + bub * ((tub_w - 24) // max(1, min(6, 2 + ht_lvl) - 1))
            bub_y2 = tub_top + 14 - int(bub_phase * 14)
            _br4 = max(2, bub_r)
            bs4 = pygame.Surface((_br4 * 2, _br4 * 2), pygame.SRCALPHA)
            pygame.draw.circle(bs4, (*wc1, bub_a), (_br4, _br4), _br4, 1)
            pygame.draw.circle(bs4, (255, 255, 255, min(255, bub_a + 40)), (_br4 - 1, _br4 - 1), max(1, _br4 // 3))
            surf.blit(bs4, (bub_x2 - bub_r, bub_y2 - bub_r))

        # ── Tub rim highlight ─────────────────────────────────────────────
        pygame.draw.rect(surf, (252, 240, 230), (tub_left + 3, tub_top, tub_w - 6, 6), border_radius=10)

        # ── Jet nozzles on wall ────────────────────────────────────────────
        for jt in range(min(4, 1 + ht_lvl)):
            jx2 = tub_left + 14 + jt * ((tub_w - 28) // max(1, min(4, 1+ht_lvl) - 1) if ht_lvl > 0 else 1)
            jy2 = tub_top + tub_h - 16
            pygame.draw.circle(surf, (144, 124, 108), (jx2, jy2), 6)
            pygame.draw.circle(surf, (200, 180, 160), (jx2, jy2), 4)
            pygame.draw.circle(surf, (100, 80, 60), (jx2, jy2), 2)

        # ── Income label ──────────────────────────────────────────────────
        ht_inc = 40 * ht_lvl
        ht_s = _font19.render(f"+{ht_inc}G/s", True, (255, 200, 120))
        surf.blit(ht_s, (tub_left, tub_top - 22))
        # Steam particles
        for s in _nw_steam:
            sr = max(2, int(s[3]))
            sa = max(0, min(255, int(s[4] * 200)))
            if sa < 5: continue
            _ss_dim = max(4, sr * 2)
            ss = pygame.Surface((_ss_dim, _ss_dim), pygame.SRCALPHA)
            pygame.draw.circle(ss, (230, 230, 255, sa), (sr, sr), sr)
            pygame.draw.circle(ss, (255, 255, 255, min(255, sa + 60)), (sr, sr), max(1, sr // 2))
            surf.blit(ss, (int(s[0]) - sr, int(s[1]) - sr))

    # ── GOLDEN FAUCET (top-left wall) ───────────────────────────────────
    if _nw_shop_bought["nw_faucet"] > 0 and not _nw_effect_off["nw_faucet"]:
        f_lvl  = _nw_shop_bought["nw_faucet"]
        fx_pos = int(W * 0.12)
        fy_pos = 72
        ...
        goldd = (140, 100, 10)
        shine = (255, 245, 160)
        plat  = (200, 200, 210) if f_lvl < 5 else (220, 220, 240)

        # Backplate with ornamental border
        pygame.draw.rect(surf, goldd, (fx_pos - 24, fy_pos - 14, 48, 28), border_radius=6)
        pygame.draw.rect(surf, gold, (fx_pos - 22, fy_pos - 12, 44, 24), border_radius=5)
        pygame.draw.rect(surf, goldb, (fx_pos - 20, fy_pos - 10, 40, 6), border_radius=3)
        # Ornamental rivets
        for rv_x in [fx_pos - 18, fx_pos + 18]:
            pygame.draw.circle(surf, goldb, (rv_x, fy_pos - 6), 3)
            pygame.draw.circle(surf, shine, (rv_x - 1, fy_pos - 7), 1)
        # Engraved pattern (cross-hatch on plate)
        for hx in range(fx_pos - 16, fx_pos + 16, 6):
            pygame.draw.line(surf, goldd, (hx, fy_pos - 6), (hx + 2, fy_pos + 8), 1)

        # ── Faucet neck with chrome plating ────────────────────────────
        pygame.draw.rect(surf, goldd, (fx_pos - 6, fy_pos + 12, 12, 30), border_radius=4)
        pygame.draw.rect(surf, gold, (fx_pos - 5, fy_pos + 13, 10, 28), border_radius=3)
        pygame.draw.line(surf, shine, (fx_pos - 2, fy_pos + 14), (fx_pos - 2, fy_pos + 38), 2)

        # ── Spout arm (horizontal + downward) ──────────────────────────
        pygame.draw.rect(surf, goldd, (fx_pos - 5, fy_pos + 38, 28, 10), border_radius=4)
        pygame.draw.rect(surf, gold, (fx_pos - 4, fy_pos + 39, 26, 8), border_radius=3)
        pygame.draw.line(surf, shine, (fx_pos - 3, fy_pos + 40), (fx_pos + 20, fy_pos + 40), 1)
        pygame.draw.rect(surf, goldd, (fx_pos + 16, fy_pos + 44, 10, 16), border_radius=3)
        pygame.draw.rect(surf, gold, (fx_pos + 17, fy_pos + 45, 8, 14), border_radius=2)
        pygame.draw.line(surf, shine, (fx_pos + 19, fy_pos + 46), (fx_pos + 19, fy_pos + 58), 1)

        # ── Tap handles (hot=red, cold=blue, gold plated at high levels) ─
        for th, handle_col, h_shine in [
            (-1, (200, 50, 50) if f_lvl < 8 else gold, (255, 100, 100) if f_lvl < 8 else goldb),
            (+1, (50,  90, 200) if f_lvl < 8 else gold, (100, 140, 255) if f_lvl < 8 else goldb),
        ]:
            hx2 = fx_pos + th * 22; hy2 = fy_pos + 4
            pygame.draw.rect(surf, gold, (hx2 - 8, hy2 - 3, 16, 8), border_radius=2)
            pygame.draw.rect(surf, handle_col, (hx2 - 6, hy2 - 5, 12, 10), border_radius=3)
            pygame.draw.rect(surf, h_shine, (hx2 - 4, hy2 - 4, 8, 4), border_radius=2)

        # ── Spout tip nozzle ────────────────────────────────────────────
        noz_x, noz_y = fx_pos + 21, fy_pos + 58
        pygame.draw.ellipse(surf, goldd, (noz_x - 6, noz_y - 3, 12, 8))
        pygame.draw.ellipse(surf, goldb, (noz_x - 5, noz_y - 2, 10, 6))

        # ── Gold coin drip particles ──────────────────────────────────────
        f_inc = 100 * f_lvl
        for d in _nw_faucet_drips:
            dr = max(2, int(d[3]))
            da = max(0, min(255, int(d[4] * 255)))
            # Outer glow
            _dr4 = max(4, dr * 4)
            gs5 = pygame.Surface((_dr4, _dr4), pygame.SRCALPHA)
            pygame.draw.circle(gs5, (255, 180, 0, da // 3), (_dr4 // 2, _dr4 // 2), _dr4 // 2)
            surf.blit(gs5, (int(d[0]) - _dr4 // 2, int(d[1]) - _dr4 // 2))
            # Coin body
            dsurf = pygame.Surface((max(2, dr * 2), max(3, dr * 3)), pygame.SRCALPHA)
            pygame.draw.ellipse(dsurf, (218, 165, 32, da), (0, 0, dr * 2, dr * 3))
            pygame.draw.ellipse(dsurf, (255, 240, 120, min(255, da + 80)),
                               (dr // 2, dr // 4, dr, dr + 2))
            # G symbol at larger sizes
            if dr >= 5:
                pygame.draw.ellipse(dsurf, (255, 200, 30, da), (1, 2, dr * 2 - 2, dr * 3 - 4), 1)
            surf.blit(dsurf, (int(d[0]) - dr, int(d[1]) - dr))

        # ── Income label + coin sparkle ──────────────────────────────────
        f_s = _font19.render(f"+{f_inc}G/s", True, (255, 220, 60))
        surf.blit(f_s, (fx_pos - 16, fy_pos + 78))
        # Shimmer on faucet
        shim_a = int(140 + 80 * math.sin(t * 3.1))
        for sx_off, sy_off in [(-2, 14), (0, 24), (2, 36)]:
            pygame.draw.circle(surf, (*shine, shim_a), (fx_pos + sx_off, fy_pos + sy_off), 2)

    # ── SOAP BUBBLES floating in the air ───────────────────────────────
    if _nw_shop_bought["nw_bubble"] > 0 and not _nw_effect_off["nw_bubble"]:
        b_lvl = _nw_shop_bought["nw_bubble"]
        # Income label near top-right of game area
        b_inc = 8 * b_lvl
        b_inc_s = _font19.render(f"BUBBLES +{b_inc}G/s", True, (140, 210, 255))
        surf.blit(b_inc_s, (NW_GAME_W - b_inc_s.get_width() - 10, 60))
        for b in _nw_bubbles:
            bx2, by2, br2 = int(b[0]), int(b[1]), max(3, int(b[2]))
            ba2 = max(0, min(255, int(b[6] * 220)))
            if ba2 < 8: continue
            # Iridescent colour cycling
            hue1 = int(b[5] + t * 50) % 360
            hue2 = (hue1 + 90) % 360
            hue3 = (hue1 + 200) % 360
            try:
                c1 = pygame.Color(0); c1.hsva = (hue1 % 360, 65, 95, 100)
                c2 = pygame.Color(0); c2.hsva = (hue2 % 360, 70, 90, 100)
                c3 = pygame.Color(0); c3.hsva = (hue3 % 360, 55, 100, 100)
            except (ValueError, TypeError):
                c1 = pygame.Color(140, 200, 255, 255)
                c2 = pygame.Color(200, 140, 255, 255)
                c3 = pygame.Color(255, 200, 140, 255)

            # ── Outer glow halo ──────────────────────────────────────────
            glow_r = br2 + 5
            gs6 = pygame.Surface((glow_r * 2, glow_r * 2), pygame.SRCALPHA)
            pygame.draw.circle(gs6, (c1.r, c1.g, c1.b, ba2 // 5), (glow_r, glow_r), glow_r)
            surf.blit(gs6, (bx2 - glow_r, by2 - glow_r))

            # ── Main bubble shell (thin iridescent outline) ───────────────
            bs2 = pygame.Surface((br2 * 2 + 2, br2 * 2 + 2), pygame.SRCALPHA)
            cx3, cy3 = br2 + 1, br2 + 1
            # Multi-layer shell for thickness illusion
            _bw = max(1, min(3, br2 - 1))
            pygame.draw.circle(bs2, (c1.r, c1.g, c1.b, ba2 // 2), (cx3, cy3), br2, _bw)
            pygame.draw.circle(bs2, (c2.r, c2.g, c2.b, ba2 // 2), (cx3, cy3), br2, max(1, _bw - 1))
            pygame.draw.circle(bs2, (c3.r, c3.g, c3.b, min(255, ba2)), (cx3, cy3), br2, 1)
            # Inner highlight arc (top-left quadrant)
            hl_r = max(1, br2 - 3)
            if hl_r >= 2:
                pygame.draw.arc(bs2, (255, 255, 255, min(255, ba2 + 60)),
                               (cx3 - hl_r - 1, cy3 - hl_r - 1, hl_r * 2, hl_r * 2),
                               math.pi * 0.6, math.pi * 1.1, max(1, min(br2 // 4, hl_r - 1)))
            surf.blit(bs2, (bx2 - br2 - 1, by2 - br2 - 1))

            # ── Specular highlights ───────────────────────────────────────
            # Primary bright spot (top-left)
            hl_off = max(2, br2 // 3)
            hl_sz  = max(1, br2 // 4)
            pygame.draw.circle(surf, (255, 255, 255, min(255, ba2 + 80)),
                               (bx2 - hl_off, by2 - hl_off), hl_sz)
            # Secondary smaller spot
            if br2 > 8:
                pygame.draw.circle(surf, (c3.r, c3.g, c3.b, min(255, ba2 + 60)),
                                   (bx2 + hl_off // 2, by2 + hl_off // 2), max(1, hl_sz - 1))

            # ── Bottom refraction glint ───────────────────────────────────
            if br2 > 10:
                pygame.draw.circle(surf, (c2.r, c2.g, c2.b, ba2 // 3),
                                   (bx2, by2 + br2 - 3), max(1, br2 // 5))

    # ── RUBBER DUCKY orbiting the bouncer ──────────────────────────────
    if _nw_shop_bought["nw_ducky"] > 0 and not _nw_effect_off["nw_ducky"]:
        lvl_dy   = _nw_shop_bought["nw_ducky"]
        orbit_r  = _nw_bo_r + 32 + min(lvl_dy, 10) * 2
        n_ducks  = min(lvl_dy, 8)
        inc_per  = 15 * lvl_dy

        for di in range(n_ducks):
            da   = _nw_ducky_angle + di * (math.pi * 2 / max(1, n_ducks))
            dkx  = int(_nw_bo_x + math.cos(da) * orbit_r)
            dky  = int(_nw_bo_y + math.sin(da) * orbit_r * 0.5 + _nw_ducky_wobble)
            flip = math.cos(da) >= 0   # face direction based on orbit side
            ds   = 11

            # ── Duck shadow ───────────────────────────────────────────────
            shad_s = pygame.Surface((ds * 4, 8), pygame.SRCALPHA)
            pygame.draw.ellipse(shad_s, (0, 0, 0, 50), (0, 2, ds * 4, 6))
            surf.blit(shad_s, (dkx - ds * 2, dky + ds // 2))

            # ── Body (plump oval) ─────────────────────────────────────────
            body_w, body_h = ds * 2 + 2, int(ds * 1.5)
            pygame.draw.ellipse(surf, (180, 130, 10), (dkx - ds - 1, dky - body_h // 2 + 1, body_w + 2, body_h + 2))
            pygame.draw.ellipse(surf, (255, 210, 20), (dkx - ds, dky - body_h // 2, body_w, body_h))
            # Body highlight
            _bhl = max(1, body_h // 2 - 2)
            _bhlw = max(1, body_w - 6)
            pygame.draw.ellipse(surf, (255, 245, 120),
                               (dkx - ds + 2, dky - body_h // 2 + 2, _bhlw, _bhl))

            # ── Wing ─────────────────────────────────────────────────────
            wing_x = (dkx + 3) if flip else (dkx - 3)
            pygame.draw.ellipse(surf, (220, 180, 10),
                               (wing_x - 6, dky - 3, 12, 8))
            pygame.draw.ellipse(surf, (255, 230, 60),
                               (wing_x - 4, dky - 2, 8, 5))

            # ── Head ──────────────────────────────────────────────────────
            head_r = ds - 2
            head_x = (dkx + ds - 2) if flip else (dkx - ds + 2)
            pygame.draw.circle(surf, (180, 130, 10), (head_x, dky - body_h // 2 + 2), head_r + 1)
            pygame.draw.circle(surf, (255, 215, 20), (head_x, dky - body_h // 2 + 2), head_r)
            pygame.draw.circle(surf, (255, 245, 120),
                               (head_x + (-2 if flip else 2), dky - body_h // 2), max(1, head_r // 2))

            # ── Beak ──────────────────────────────────────────────────────
            bk_x = (head_x + head_r + 2) if flip else (head_x - head_r - 2)
            bk_y = dky - body_h // 2 + 2
            pts_bk = ([(bk_x, bk_y - 3), (bk_x + 7, bk_y), (bk_x, bk_y + 3)]
                      if flip else [(bk_x, bk_y - 3), (bk_x - 7, bk_y), (bk_x, bk_y + 3)])
            pygame.draw.polygon(surf, (220, 100, 10), pts_bk)
            pygame.draw.polygon(surf, (255, 140, 30), pts_bk, 1)

            # ── Eye ───────────────────────────────────────────────────────
            eye_x = (head_x + 2) if flip else (head_x - 2)
            eye_y = bk_y - 3
            pygame.draw.circle(surf, (10, 10, 10), (eye_x, eye_y), 2)
            pygame.draw.circle(surf, (255, 255, 255), (eye_x + (1 if flip else -1), eye_y - 1), 1)

            # ── Crown at high levels ──────────────────────────────────────
            if lvl_dy >= 5:
                crown_y = dky - body_h // 2 + 2 - head_r - 2
                crown_col = (255, 200, 30)
                crown_pts = [
                    (head_x - 6, crown_y + 4),
                    (head_x - 4, crown_y),
                    (head_x - 1, crown_y + 3),
                ...
                pygame.draw.polygon(surf, crown_col, crown_pts)
                pygame.draw.polygon(surf, (255, 240, 100), crown_pts, 1)
                # Jewels
                for jpos in [(head_x - 1, crown_y + 1), (head_x + 5, crown_y + 1)]:
                    pygame.draw.circle(surf, (200, 30, 30), jpos, 2)

        # ── Orbit ring ────────────────────────────────────────────────────
        or_s = pygame.Surface((orbit_r * 2 + 4, int(orbit_r + 4)), pygame.SRCALPHA)
        pygame.draw.ellipse(or_s, (255, 215, 0, 22),
                           (0, 0, orbit_r * 2 + 4, int(orbit_r + 4)), 1)
        surf.blit(or_s, (int(_nw_bo_x) - orbit_r - 2, int(_nw_bo_y) - orbit_r // 2 - 2))

        # ── Income label ──────────────────────────────────────────────────
        dy_s = _font19.render(f"+{inc_per}G/bounce", True, (255, 215, 30))
        surf.blit(dy_s, (int(_nw_bo_x) - orbit_r - 2, int(_nw_bo_y) + orbit_r // 2 + 6))
    ...
    if sz>=22:
        lbl_s=_font22.render("BOUNCE",True,(0,0,0))
    ...
    # ── LEFT HUD: green coin balance + passive income rate ───────────────
    pygame.draw.circle(surf,(50,200,70),(24,22),12)
    pygame.draw.circle(surf,(120,255,140),(24,22),9,2)
    gc_big_f=_nw_sysfont("Arial",44,bold=True)
    gc_surf=gc_big_f.render(fmt_nw_coins(green_coins),True,(80,240,110))
    surf.blit(gc_surf,(42,10))
    passive = _nw_passive_income_per_sec()
    bounce_b = _nw_bounce_bonus()
    surf.blit(_font19.render("GREEN COINS",True,(60,170,80)),(14,44))
    if passive > 0:
        surf.blit(_font19.render(f"+{passive}G/s passive", True, (100, 220, 130)), (14, 60))
    if bounce_b > 1:
        surf.blit(_font19.render(f"+{bounce_b}G/bounce", True, (255, 215, 50)), (14, 76))
    ...
    all_lbl=_font18.render("ALL",True,(255,255,180) if _nw_all_goons else (200,190,140))
    ...
    for i, trk in enumerate(NW_SHOP_TRACKS):
        ...
        is_money   = act not in ("nw_speed", "nw_size")
        ...
        # ── Row background ───────────────────────────────────────────────
        if at_max:
            bg = (24, 38, 28) if is_money else (28, 28, 32)
        elif can_afford:
            bg = bg_base
        else:
            bg = (26, 26, 30)
        pygame.draw.rect(surf, bg, row_rect, border_radius=8)

        # ── Animated border ──────────────────────────────────────────────
        if at_max and is_money:
            # Gold shimmer for maxed income upgrades
            gp = int(160 + 60 * math.sin(shop_t * 3.0 + i))
            pygame.draw.rect(surf, (gp, int(gp*0.8), 0), row_rect, 2, border_radius=8)
        elif at_max:
            pygame.draw.rect(surf, (50, 180, 80), row_rect, 1, border_radius=8)
        elif can_afford:
            pulse = int(140 + 90 * math.sin(shop_t * 2.4 + i * 1.1))
            bc    = tuple(min(255, int(c * pulse / 230)) for c in accent)
            pygame.draw.rect(surf, bc, row_rect, 2, border_radius=8)
        else:
            pygame.draw.rect(surf, (180, 50, 50), row_rect, 1, border_radius=8)

        # ── Left accent stripe ───────────────────────────────────────────
        stripe_c = accent if (can_afford or at_max) else (55, 55, 62)
        pygame.draw.rect(surf, stripe_c, (row_rect.x, ry+5, 4, ROW_H-10), border_radius=2)

        # ── Icon box — detailed pixel art per upgrade ─────────────────────
        icon_bg = pygame.Rect(row_rect.x+6, ry+5, 42, ROW_H-10)
        ib_col  = (16, 18, 22) if (can_afford and not at_max) else (32, 34, 38)
        pygame.draw.rect(surf, ib_col, icon_bg, border_radius=9)
        pygame.draw.rect(surf, accent if (can_afford and not at_max) else (60, 62, 68),
                         icon_bg, 1, border_radius=9)
        icx, icy = icon_bg.centerx, icon_bg.centery
        ic  = accent if (can_afford and not at_max) else (80, 82, 90)
        dim = (100, 100, 110)

        if act == "nw_speed":
            # Lightning bolt
            pts = [(icx-4,icy-11),(icx+2,icy-2),(icx-1,icy-2),(icx+4,icy+11),(icx-2,icy+2),(icx+1,icy+2)]
            pygame.draw.polygon(surf, ic, pts)
            pygame.draw.polygon(surf, tuple(min(255,c+60) for c in ic), pts, 1)
        elif act == "nw_size":
            # Expanding squares
            pygame.draw.rect(surf, ic, (icx-10, icy-10, 20, 20), 2, border_radius=2)
            pygame.draw.rect(surf, tuple(min(255,c+80) for c in ic), (icx-5, icy-5, 10, 10), 0, border_radius=1)
            for corner_dx, corner_dy in [(-12,-12),(8,-12),(-12,8),(8,8)]:
                pygame.draw.line(surf, ic, (icx+corner_dx, icy+corner_dy),
                                 (icx+corner_dx+(3 if corner_dx>0 else -3), icy+corner_dy+(3 if corner_dy>0 else -3)), 2)
        elif act == "nw_bubble":
            # Iridescent soap bubbles with shine
            for bpos, br2, hoff in [(-6, 7, 0), (5, 5, 120), (0, -5, 240)]:
                hc = pygame.Color(0); hc.hsva = ((int(shop_t*60)+hoff)%360, 70, 100, 100)
                bc2 = (hc.r, hc.g, hc.b)
                pygame.draw.circle(surf, bc2, (icx+bpos, icy+br2), abs(br2), 1)
                pygame.draw.circle(surf, (255,255,255), (icx+bpos-2, icy+br2-2), max(1,abs(br2)//3))
        elif act == "nw_ducky":
            # Detailed rubber ducky
            gold = (255, 210, 20) if (can_afford and not at_max) else (140, 120, 40)
            golddark = (200, 160, 10) if (can_afford and not at_max) else (100, 90, 30)
            orange = (255, 130, 20) if (can_afford and not at_max) else (120, 80, 30)
            # body
            pygame.draw.ellipse(surf, gold, (icx-10, icy-1, 20, 13))
            pygame.draw.ellipse(surf, golddark, (icx-10, icy-1, 20, 13), 1)
            # head
            pygame.draw.circle(surf, gold, (icx+7, icy-5), 7)
            pygame.draw.circle(surf, golddark, (icx+7, icy-5), 7, 1)
            # beak
            pygame.draw.polygon(surf, orange, [(icx+13,icy-6),(icx+18,icy-4),(icx+13,icy-2)])
            # eye
            pygame.draw.circle(surf, (10,10,10), (icx+9, icy-7), 2)
            pygame.draw.circle(surf, (255,255,255), (icx+10, icy-8), 1)
            # wing highlight
            pygame.draw.ellipse(surf, (255,240,100) if (can_afford and not at_max) else golddark, (icx-5, icy+1, 9, 5))
        elif act == "nw_hottub":
            # Hot tub with steam and glowing water
            wc = (int(60+40*math.sin(shop_t*2)), int(120+40*math.sin(shop_t*1.5+1)), 200)
            # tub body
            pygame.draw.rect(surf, (160,130,110) if (can_afford and not at_max) else (80,70,60),
                             (icx-11, icy-1, 22, 14), border_radius=4)
            # water
            pygame.draw.ellipse(surf, wc, (icx-9, icy-1, 18, 10))
            # steam wisps
            for si2, (sox, soy) in enumerate([(-5,-5),(0,-8),(5,-5)]):
                sa = int(180 * abs(math.sin(shop_t*1.8 + si2*0.7)))
                for dy in range(0, 8, 2):
                    pygame.draw.line(surf, (*ic, max(0, sa-dy*20)),
                                     (icx+sox + int(math.sin((shop_t+si2)*2+dy)*2), icy+soy-dy),
                                     (icx+sox + int(math.sin((shop_t+si2)*2+dy+1)*2), icy+soy-dy-2), 1)
            # rim
            pygame.draw.rect(surf, ic, (icx-11, icy-1, 22, 14), 1, border_radius=4)
        elif act == "nw_disco":
            # Spinning disco ball with coloured light beams
            ball_r = 8
            # Hanging wire
            pygame.draw.line(surf, ic, (icx, icy-14), (icx, icy-ball_r-2), 1)
            # Ball facets
            for ring in range(3):
                for seg in range(6):
                    ang = seg * math.pi / 3 + shop_t * 1.8 + ring * 0.5
                    bri = int(100 + 120 * abs(math.sin(ang + shop_t)))
                    fc = (bri, bri, bri)
                    x1 = int(icx + math.cos(ang) * (ball_r - ring*2))
                    y1 = int(icy - 3 + math.sin(ang) * 3 + ring*2)
                    x2 = int(icx + math.cos(ang + math.pi/3) * (ball_r - ring*2))
                    y2 = int(icy - 3 + math.sin(ang + math.pi/3) * 3 + ring*2)
                    pygame.draw.line(surf, fc, (x1,y1), (x2,y2), 1)
            pygame.draw.circle(surf, (180,180,180), (icx, icy-3), ball_r, 1)
            # Light beams
            for bi in range(4):
                ba = bi * math.pi/2 + shop_t * 1.2
                col_b = NW_DISCO_COLS[bi % len(NW_DISCO_COLS)]
                ba2 = int(80 + 60 * math.sin(shop_t*2.5 + bi))
                ex = int(icx + math.cos(ba) * 14)
                ey = int(icy - 3 + math.sin(ba) * 10)
                if can_afford and not at_max:
                    pygame.draw.line(surf, col_b, (icx, icy-3), (ex, ey), 1)
        elif act == "nw_faucet":
            # Golden faucet with dripping coin drops
            gold2 = (255,200,30) if (can_afford and not at_max) else (120,100,30)
            shine = (255,240,140) if (can_afford and not at_max) else (140,120,60)
            dark2 = (180,140,20) if (can_afford and not at_max) else (90,80,20)
            # Mount plate
            pygame.draw.rect(surf, dark2, (icx-10, icy-13, 20, 10), border_radius=2)
            pygame.draw.rect(surf, gold2, (icx-9, icy-12, 18, 8), border_radius=2)
            # Neck
            pygame.draw.rect(surf, gold2, (icx-3, icy-5, 6, 10), border_radius=2)
            pygame.draw.line(surf, shine, (icx-1, icy-4), (icx-1, icy+3), 1)
            # Spout
            pygame.draw.rect(surf, gold2, (icx-3, icy+3, 14, 5), border_radius=2)
            pygame.draw.rect(surf, gold2, (icx+7, icy+6, 5, 7), border_radius=2)
            # Animated drip
            dp = (shop_t * 1.6 + i) % 1.0
            if dp < 0.7:
                dy2 = int(icy + 12 + dp * 18)
                dr2 = max(2, int(3 * (1 - dp)))
                ds3 = pygame.Surface((dr2*2, dr2*3), pygame.SRCALPHA)
                pygame.draw.ellipse(ds3, (*gold2, int((1-dp)*220)), (0,0,dr2*2,dr2*3))
                surf.blit(ds3, (icx+8, dy2))

        # ── Progress bar — compact for uncapped, capped for stats ────────
        bar_x  = row_rect.x + 56
        bar_y  = ry + ROW_H - 13
        if is_money:
            # Simple horizontal fill bar with count label
            bar_w   = row_rect.right - bar_x - 60
            fill_f  = min(1.0, bought / max(1, min(bought+20, bought+20))) if bought > 0 else 0
            # Show last 20 purchases as a bar
            show    = min(20, bought)
            bar_filled_w = int(bar_w * show / 20) if show < 20 else bar_w
            pygame.draw.rect(surf, (40,44,50), (bar_x, bar_y-3, bar_w, 6), border_radius=2)
            if show > 0:
                pygame.draw.rect(surf, accent, (bar_x, bar_y-3, bar_filled_w, 6), border_radius=2)
            # Purchase count badge
            count_s = fn_tiny.render(f"×{bought}", True, accent if bought>0 else (60,62,68))
            surf.blit(count_s, (row_rect.right - count_s.get_width() - 46, bar_y - 7))
        else:
            # Dot progress for capped stats
            dot_r = 4; dot_gap = 10
            for d in range(cap):
                dx2 = bar_x + d * dot_gap
                if dx2 + dot_r > row_rect.right - 60: break
                if d > 0 and d % NW_UNLOCK_THRESH == 0:
                    pygame.draw.line(surf, (80,80,90), (dx2-3, bar_y-5), (dx2-3, bar_y+4), 1)
                pygame.draw.circle(surf, accent if d < bought else (44,46,52),
                                   (dx2, bar_y), dot_r if d < bought else dot_r-1,
                                   0 if d < bought else 1)

        # ── Name + income label ──────────────────────────────────────────
        tc = (235,238,242) if (can_afford and not at_max) else (200,220,200) if at_max else (80,82,90)
        name_s = fn_mid.render(trk["name"], True, tc)
        surf.blit(name_s, (row_rect.x+56, ry+6))

        # Income / stat value as a bright label
        if act == "nw_bubble":
            inc_str = f"+{8*bought}G/s" if bought>0 else "+8G/s per buy"
        elif act == "nw_ducky":
            inc_str = f"+{15*bought}G/bounce" if bought>0 else "+15G/bounce per buy"
        elif act == "nw_hottub":
            inc_str = f"+{40*bought}G/s" if bought>0 else "+40G/s per buy"
        elif act == "nw_disco":
            inc_str = f"+{25*bought}G/bounce" if bought>0 else "+25G/bounce per buy"
        elif act == "nw_faucet":
            inc_str = f"+{100*bought}G/s" if bought>0 else "+100G/s per buy"
        elif act == "nw_speed":
            inc_str = f"+{bought*5}% spd" if bought>0 else "+5% speed"
        else:
            inc_str = f"+{bought*5}% sz" if bought>0 else "+5% size"
        inc_col = (100,255,140) if bought>0 else (80,120,90)
        inc_s   = fn_tiny.render(inc_str, True, inc_col)
        surf.blit(inc_s, (row_rect.x+56, ry+26))

        # ── Price or MAXED ────────────────────────────────────────────────
        if at_max and not is_money:
            pp_surf = fn_sm.render("MAX", True, (60,200,100))
            surf.blit(pp_surf, (row_rect.right - pp_surf.get_width() - 10, ry+8))
        elif not at_max:
            pp   = fmt_nw_coins(price)
            pc2  = (80,255,120) if can_afford else (255,80,80)
            ps_s = fn_sm.render(pp, True, pc2)
            surf.blit(ps_s, (row_rect.right - ps_s.get_width() - 10, ry+8))

        # ── Toggle ON/OFF button ──────────────────────────────────────────
        if bought > 0:
            tog_bg  = (44,24,24) if effect_off else (24,42,26)
            tog_brd = (200, 60, 60) if effect_off else (60, 210, 80)
            tog_hov = tog_rect.collidepoint(mx, my)
            if tog_hov:
                tog_bg = tuple(min(255, c+24) for c in tog_bg)
            pygame.draw.rect(surf, tog_bg,  tog_rect, border_radius=6)
            pygame.draw.rect(surf, tog_brd, tog_rect, 2, border_radius=6)
            tl = fn_tiny.render("OFF" if effect_off else "ON", True, tog_brd)
            surf.blit(tl, tl.get_rect(center=tog_rect.center))
...
async def new_world_cinematic(screen, clock, W, H, font):
    ...
    while True:
                ...
                if left_r.collidepoint(mx2, my2) and bouncers:
                    ...
                    for buy_rect, tog_rect, trk in shop_items:
                        ...
                        if tog_rect.collidepoint(mx2, my2) and _nw_shop_bought[act] > 0:
                            ...
                            # Clear stale particles when toggling OFF
                            if _nw_effect_off[act]:
                                if act == "nw_bubble":  _nw_bubbles.clear()
                                if act == "nw_hottub":  _nw_steam.clear()
                                if act == "nw_faucet":  _nw_faucet_drips.clear()
                                if act == "nw_disco":   _nw_disco_flash.clear()
                            _nw_apply_bonuses()
                            break
                        # Buy button
                        if buy_rect.collidepoint(mx2, my2):
                            bought_now = _nw_shop_bought[act]
                            if bought_now < _nw_cap(act):
                                price = _nw_shop_price(act)
                                if green_coins >= price or cheat_mode:
                                    ...
                                    # nw_bubble / nw_ducky / nw_hottub / nw_disco / nw_faucet
                                    # all take effect immediately via _nw_update_effects
                            break
        ...
        if in_bath:
            ...
            # Physics — earn green coins per wall bounce (base + upgrades)
            bo_cx_i, bo_cy_i, hit_wall = _nw_update_bouncer(dt, floor_y_nw, W, H)
            if hit_wall:
                green_coins += _nw_bounce_bonus()

            # Update all visual effects (bubbles, ducky, steam, disco, faucet)
            try:
                _nw_update_effects(dt, t_bath, bo_cx_i, bo_cy_i, floor_y_nw, W, H, hit_wall)
            except Exception:
                pass
            ...
            except Exception as _nw_draw_err:
                # Fallback: plain background so game doesn't crash
                screen.fill((180, 175, 168))
                _fb = _font19.render(f"[draw err: {type(_nw_draw_err).__name__}]", True, (200, 80, 80))
                screen.blit(_fb, (20, H // 2))
    ...
    while True:
        ...
        # ========== CODE ENTRY ==========
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if _fx_btn_rect.collidepoint(mx, my):
                        ...
                        if not fx_enabled:
                            ...
                            implosion_effects.clear()
                            for _b in bouncers: _b.trail_points.clear()
                        continue
            ...
            if pending_new_world and new_world_task is None:
                pending_new_world = False
                new_world_task = True   # mark as running
            ...
            all_lbl = _font18.render("ALL", True, (255, 255, 180) if all_goons_mode else (200, 190, 140))
