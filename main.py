...
BASE_SHOP = [
    ...
    {"name": "God Goon",                "base_price": 450000000000000000,"action": "goongod"},
]
...
GOON_SHOP = [
    ...
    {"name": "Goon God Goon",           "base_price": 450000000000000000,"action": "goongod"},
]
...
class Factory:
    __slots__ = ('x', 'phase', 'smoke_puffs', 'gear_angle', 'spark_timer', 'sparks')
    BODY_W  = 90
    BODY_H  = 110
    ROOF_H  = 22
    ...
    def __init__(self, x):
        ...
        self.gear_angle  = 0.0
        self.spark_timer = 0
        self.sparks      = []   # list of [sx,sy,vx,vy,life]
        self.smoke_puffs = [
            [self.x + random.randint(-25, 25),
             HEIGHT - self.BODY_H - self.ROOF_H - random.randint(10, 60),
    ...
    # ── Internal helpers ─────────────────────────────────────────
    def _draw_gear(self, surface, cx, cy, r, teeth, angle, col):
        tooth_r = r + 5
        inner_r = r - 4
        ...
        for k in range(teeth * 2):
            a   = angle + k * math.pi / teeth
            rad = tooth_r if k % 2 == 0 else inner_r
            pts_outer.append((cx + math.cos(a) * rad, cy + math.sin(a) * rad))
        draw_polygon(surface, col, [(int(x), int(y)) for x, y in pts_outer])
        draw_circle(surface, (20, 20, 20), (cx, cy), inner_r - 2)
        draw_circle(surface, col,          (cx, cy), 5)

    def update(self, dt, now):
        self.gear_angle += 1.8 * dt
        # Update smoke puffs
        for s in self.smoke_puffs:
            s[0] += s[2]; s[1] += s[3]
            s[4] -= 0.008
            if s[4] <= 0 or s[1] < HEIGHT - self.BODY_H - self.ROOF_H - 120:
                s[0] = self.x + random.randint(-20, 20)
                s[1] = HEIGHT - self.BODY_H - self.ROOF_H - 4
                s[2] = random.uniform(-0.4, 0.4)
                s[3] = random.uniform(-1.0, -0.4)
                s[4] = random.uniform(0.5, 1.0)
        # Sparks from windows
        self.spark_timer += dt
        if self.spark_timer > 0.12:
            self.spark_timer = 0
            if len(self.sparks) < 20:
                wx = self.x + random.choice([-22, 0, 22])
                wy = HEIGHT - self.BODY_H // 2
                self.sparks.append([float(wx), float(wy),
                                    random.uniform(-60, 60),
                                    random.uniform(-90, -30),
                                    1.0])
        i = 0
        while i < len(self.sparks):
            sp = self.sparks[i]
            sp[0] += sp[2] * dt; sp[1] += sp[3] * dt
            sp[3] += 80 * dt    # gravity
            sp[4] -= 2.5 * dt
            if sp[4] <= 0:
                self.sparks[i] = self.sparks[-1]; self.sparks.pop()
            else:
                i += 1
    ...
    def draw(self, surface, now):
        ...
        # ── Ground shadow ──────────────────────────────────────────
        sh_surf = pygame.Surface((bw + 30, 14), pygame.SRCALPHA)
        pygame.draw.ellipse(sh_surf, (0, 0, 0, 80), (0, 4, bw + 30, 10))
        surface.blit(sh_surf, (bx - 15, HEIGHT - 12))

        # ── Main body ─────────────────────────────────────────────
        steel_dark  = (45, 55, 65)
        steel_mid   = (70, 85, 100)
        steel_light = (100, 120, 140)
        accent      = (255, 140, 0)
        accent_dim  = (180, 80, 0)

        pygame.draw.rect(surface, steel_dark,  (bx,     by,     bw,     bh),     border_radius=3)
        pygame.draw.rect(surface, steel_mid,   (bx + 4, by + 4, bw - 8, bh - 8))
        # vertical panel lines
        for px in (bx + bw//4, bx + bw//2, bx + 3*bw//4):
            pygame.draw.line(surface, steel_dark, (px, by + 8), (px, HEIGHT - 4), 2)
        # horizontal bands
        for band_y in (by + bh//3, by + 2*bh//3):
            pygame.draw.rect(surface, steel_dark, (bx, band_y, bw, 6))
            pygame.draw.rect(surface, accent_dim, (bx, band_y + 1, bw, 2))
        # top rim highlight
        pygame.draw.line(surface, steel_light, (bx, by + 2), (bx + bw, by + 2), 2)

        # ── Roof / sawtooth silhouette ────────────────────────────
        roof_pts = []
        teeth_n  = 5
        tw       = bw // teeth_n
        for k in range(teeth_n):
            rx = bx + k * tw
            roof_pts.append((rx,      by))
            roof_pts.append((rx + tw//2, by - rh))
        roof_pts.append((bx + bw, by))
        pygame.draw.polygon(surface, steel_dark, roof_pts)
        pygame.draw.polygon(surface, steel_mid,  roof_pts, 2)

        # ── Chimneys (3) ──────────────────────────────────────────
        chimney_xs = [self.x - 28, self.x, self.x + 28]
        for chi, chx in enumerate(chimney_xs):
            ch_w = 14; ch_h = 30 + chi * 8
            ch_bx = chx - ch_w // 2
            ch_by = by - ch_h
            pygame.draw.rect(surface, steel_dark,  (ch_bx,     ch_by,     ch_w,     ch_h))
            pygame.draw.rect(surface, steel_light, (ch_bx,     ch_by,     ch_w,     4))     # rim
            pygame.draw.rect(surface, steel_light, (ch_bx,     ch_by,     2,        ch_h))  # highlight

        # ── Windows (glowing orange/yellow) ───────────────────────
        win_positions = [(-24, bh//4 - 8), (0, bh//4 - 8), (24, bh//4 - 8),
                         (-24, bh//2),     (0, bh//2),     (24, bh//2)]
        for wx_off, wy_off in win_positions:
            wxp  = self.x + wx_off - 9
            wyp  = by + wy_off
            flick = 0.75 + 0.25 * math.sin(t * 7.3 + wx_off * 0.4)
            wc   = (int(255 * flick), int(160 * flick), 0)
            # glow behind window
            gs_w = pygame.Surface((22, 22), pygame.SRCALPHA)
            pygame.draw.ellipse(gs_w, (*wc, 60), (0, 0, 22, 22))
            surface.blit(gs_w, (wxp - 2, wyp - 2))
            pygame.draw.rect(surface, (20, 20, 20), (wxp, wyp, 18, 14))
            pygame.draw.rect(surface, wc,           (wxp + 2, wyp + 2, 14, 10))
            pygame.draw.rect(surface, (200,200,200),(wxp, wyp, 18, 14), 1)

        # ── Large gear on face ────────────────────────────────────
        gear_col_outer = (int(180 + 40 * math.sin(t*2.1)), int(130 + 30 * math.sin(t*1.7)), 20)
        self._draw_gear(surface, self.x, by + 3*bh//4, 18, 8,
                        self.gear_angle, gear_col_outer)

        # ── Entrance arch ─────────────────────────────────────────
        aw = 24; ah = 30
        ax = self.x - aw // 2
        ay = HEIGHT - ah
        pygame.draw.rect(surface, (10, 10, 10), (ax, ay, aw, ah))
        pygame.draw.arc(surface, steel_light,
                        (ax, ay - ah//2, aw, ah), 0, math.pi, 2)
        # door glow
        dg = pygame.Surface((aw, ah), pygame.SRCALPHA)
        dp = 0.4 + 0.4 * math.sin(t * 4.0 + self.phase)
        dg.fill((int(255*dp), int(120*dp), 0, int(80*dp)))
        surface.blit(dg, (ax, ay))

        # ── Sparks ───────────────────────────────────────────────
        for sp in self.sparks:
            sa  = max(0, sp[4])
            scol = (int(255*sa), int(200*sa), 0)
            pygame.draw.circle(surface, scol, (int(sp[0]), int(sp[1])), max(1, int(3*sa)))

        # ── Smoke puffs ───────────────────────────────────────────
        for s in self.smoke_puffs:
            sr2 = max(2, int(14 * s[4]))
            sal = max(0, int(140 * s[4]))
            sg  = pygame.Surface((sr2*2, sr2*2), pygame.SRCALPHA)
            gc  = int(120 + 60 * s[4])
            pygame.draw.circle(sg, (gc, gc, gc, sal), (sr2, sr2), sr2)
            surface.blit(sg, (int(s[0]) - sr2, int(s[1]) - sr2))

        # ── Orange accent stripe at base ──────────────────────────
        pygame.draw.rect(surface, accent, (bx, HEIGHT - 8, bw, 8))
        pulse_w = int(bw * (0.4 + 0.6 * abs(math.sin(t * 1.5 + self.phase))))
        pygame.draw.rect(surface, (255, 220, 80), (bx + (bw - pulse_w)//2, HEIGHT - 5, pulse_w, 3))
...
def update_factories(dt, now):
    ...
    if now - _factory_income_timer >= 1000:
        coins += len(factories) * FACTORY_INCOME_PER_SEC
        _factory_income_timer = now
...
class GravityWellEffect:
    ...
    def update(self):
        ...
        if now - self.income_timer >= 1000:
            coins += GRAVITY_INCOME_PER_SEC * self.bouncer.gravity_purchases
            self.income_timer = now
...
class Bouncer:
    __slots__ = ('size','fx','fy','rect','color','speed_x','speed_y','selected',
                 ...
                 'trail_points','last_trail_income','trail_income','wave_income',
    ...
    def move(self, dt):
        ...
        if self.implosion_frozen:
            ...
            if trail_enabled_eff and now - self.last_trail_income >= 1000:
                coins += trail_income_eff * mul
                self.last_trail_income = now
            return
        ...
        # ========== GAME ==========
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    for i, item in enumerate(selected_bouncer.shop_data):
                        ...
                        if not cheat_mode and coins < item["price"]: continue
                        if not cheat_mode: coins -= item["price"]
                        item["bought"] += 1
                        selected_bouncer.increase_price(item)
                        click_animations.append({"rect": row_rect.copy(), "time": pygame.time.get_ticks()})
                        if act == "speed":
                            speed_item = next(it for it in selected_bouncer.shop_data if it["action"]=="speed")
                            if speed_item["bought"] <= 25:
                                MAX_SPEED = 2500.0
                                nx = selected_bouncer.speed_x * 1.05
                                ny = selected_bouncer.speed_y * 1.05
                                spd = math.hypot(nx, ny)
                                if spd > MAX_SPEED:
                                    scale = MAX_SPEED / spd
                                    nx *= scale; ny *= scale
                                selected_bouncer.speed_x = nx
                                selected_bouncer.speed_y = ny
                        elif act == "size":
                            cx = selected_bouncer.rect.centerx; cy = selected_bouncer.rect.centery
                            selected_bouncer.size = int(selected_bouncer.size * 1.05)
                            selected_bouncer.rect.width = selected_bouncer.rect.height = selected_bouncer.size
                            selected_bouncer.rect.center = (cx, cy)
                            selected_bouncer.draw_rect.width = selected_bouncer.draw_rect.height = selected_bouncer.size
                            selected_bouncer.draw_rect.center = (cx, cy)
                            selected_bouncer.fx = float(selected_bouncer.rect.x)
                            selected_bouncer.fy = float(selected_bouncer.rect.y)
                        elif act == "flash":
                            selected_bouncer.flashing = True
                            selected_bouncer.flash_purchases += 1
                            selected_bouncer.flash_interval = 5000 // selected_bouncer.flash_purchases
                            selected_bouncer.color = selected_bouncer._random_color()
                        elif act == "bonus":
                            if len(bouncers) < 20:
                                bouncers.append(Bouncer(GAME_WIDTH//3, HEIGHT//3))
                        elif act == "jew":
                            selected_bouncer.coin_bonus += 1
                        elif act == "wave":
                            selected_bouncer.waves_enabled = True
                            selected_bouncer.wave_income  += 100
                        elif act == "laser":
                            selected_bouncer.laser_enabled   = True
                            selected_bouncer.laser_purchases += 1
                        elif act == "trail":
                            selected_bouncer.trail_enabled = True
                            selected_bouncer.trail_income  += 5
                        elif act == "implosion":
                            if not selected_bouncer.implosion_enabled:
                                selected_bouncer.implosion_enabled = True
                                implosion_effects.append(ImplosionEffect(selected_bouncer))
                        elif act == "lightning":
                            selected_bouncer.lightning_enabled = True
                            selected_bouncer.lightning_purchases += 1
                        elif act == "factory":
                            # Space factories evenly across the bottom
                            n = len(factories) + 1
                            new_factories = []
                            for fi in range(n):
                                fx = int(GAME_WIDTH * (fi + 1) / (n + 1))
                                new_factories.append(Factory(fx))
                            factories.clear()
                            factories.extend(new_factories)
                        elif act == "illuminate":
                            selected_bouncer.illuminate_enabled = True
                            # Reposition all illuminate effects evenly across game area
                            ...
                            selected_bouncer.gravity_enabled = True
                            selected_bouncer.gravity_purchases += 1
                            # Add or refresh the GravityWellEffect for this bouncer
                            existing = [e for e in gravity_effects if e.bouncer is selected_bouncer]
                            if not existing:
                                gravity_effects.append(GravityWellEffect(selected_bouncer))
                        elif act == "mode3d":
                            selected_bouncer.mode3d_enabled = True
                            mode3d_active = True
                            mode3d_effect = Mode3DEffect()
                        elif act == "donut":
                            selected_bouncer.donut_enabled = True
                            selected_bouncer.donut_ring_count += 1
                        elif act == "goongod":
                            selected_bouncer.goon_god_enabled = True
                            selected_bouncer.goon_god_purchases += 1
            ...
            for i, item in enumerate(selected_bouncer.shop_data):
                ...
                if rect.bottom < SHOP_PANEL_TOP or rect.top > HEIGHT:
                    continue
                unlocked = selected_bouncer.is_unlocked(i)
                act      = item["action"]
                ...
                # ── Build label text ───────────────────────────────────────────────
                if act == "flash" and selected_bouncer.flashing:
                    ...
                    np = f"{item['name']} x{item['bought']}  [{cnt_total}/{DONUT_GOON_MAX} ring • own:{cnt_own} • +25 all/upg]"
                elif act == "goongod" and selected_bouncer.goon_god_enabled:
                    gp = total_goon_god_power()
                    np = f"{item['name']} x{item['bought']}  [ZEUS x{gp} • white spray]"
                else:
                ...
                ns = hud_surf(f"n{i}", np,  font, tc)
                ps = hud_surf(f"p{i}", pp,  font, pc)
            ...
            if cheat_mode:
                screen.blit(hud_surf("dev", "\u2605 DEV MODE \u2605", font, GOLD), (20, 78))
            if goon_mode:
                screen.blit(hud_surf("goon", "\U0001f4a6 GOON MODE \U0001f4a6", font, (100,255,150)), (20, 78 if not cheat_mode else 98))
